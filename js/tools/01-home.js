...
registerTool({
    ...
    <div class="text-center md:text-left flex-1 z-10">
        ...
        <div id="guide-list" class="space-y-4">
            <div class="text-center p-8 text-orange-400 font-bold animate-pulse">Đang tải bài viết... ⏳</div>
        </div>
    ...
    logic: function() {
        ...
        let loadedGuides = []; // Mảng sẽ chứa dữ liệu thật sau khi fetch

        // 🟢 BẢNG ĐIỀU KHIỂN BÀI VIẾT: 
        // Mỗi khi bạn có bài mới, chỉ cần thả file .md vào thư mục posts/ và thêm 1 dòng vào đây!
        const manifest = [
        ...
        const fetchAllMarkdown = async () => {
            try {
                // Duyệt qua manifest và tải từng file
                ...
                renderGuideList(); // Xây dựng giao diện
        ...
        const renderGuideList = () => {
            guideList.innerHTML = ''; // Xóa chữ "Đang tải"
        ...
        window.toggleGuide = function(index) {
            ...
            if (contentDiv.classList.contains('hidden')) {
                ...
                // Chỉ parse Markdown 1 lần duy nhất cho mỗi bài để tối ưu hiệu năng
                if (renderDiv.innerHTML.trim() === '') {
                    if (window.marked) {
                        ...
                        // Fix style cho các link trong bài
