    ...
    window.toggleGuide = async function(index, skipUrlUpdate = false) {
        ...
        if (contentDiv.classList.contains('hidden')) {
            ...
            if (!cachedContent[index]) {
                renderDiv.innerHTML = '<div class="text-orange-500 font-bold animate-pulse text-center py-6">Đang nạp dữ liệu bài viết...</div>';
                try {
                    ...
                    cachedContent[index] = `<div class="text-red-500 bg-red-50 p-4 text-center rounded-xl border border-red-100 my-4">Không thể tải nội dung file</div>`;
                }
            }
            ...
            setTimeout(() => {
                if (parentItem) {
                    const y = parentItem.getBoundingClientRect().top + window.scrollY - 20;
