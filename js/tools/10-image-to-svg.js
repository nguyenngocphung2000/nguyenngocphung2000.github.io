export function setupTool() {
    ...
    // Xây dựng giao diện bằng Tailwind CSS, đồng bộ chuẩn với cấu trúc Tool 06
    panel.innerHTML = '<style>' +
          ...
          // Thêm CSS của CropperJS
          '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.6.1/cropper.min.css" />' +

          '<div class="text-center mb-6">' +
          '<span class="bg-blue-100 text-blue-600 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border border-blue-200">Công cụ xử lý ảnh</span>' +
          '<h2 class="text-3xl font-bold mt-2 text-slate-800">Chuyển Ảnh Sang <span class="text-blue-500">Vector SVG</span></h2>' +
          '<p class="text-[11px] text-slate-500 italic mt-2 max-w-2xl mx-auto px-4">Chuyển đổi hình ảnh (JPG/PNG) thành mã code vector (SVG). Phóng to vô hạn không vỡ nét, tối ưu cho Web. Khuyên dùng tốt nhất cho Logo, Icon và Hình minh hoạ.</p>' +
          '</div>' +
          ...
          '<div class="bg-white/90 backdrop-blur-md p-5 rounded-3xl shadow-sm border border-slate-100 space-y-4 svg-card">' +
          ...
          '<div class="w-full h-[320px] bg-slate-100 rounded-xl overflow-hidden flex items-center justify-center relative svg-preview-box">' +
          ...
          '<ul class="text-[10px] text-slate-500 space-y-1.5 list-disc pl-4 mt-2">' +
          '<li><strong class="text-slate-700">2 - 4 màu:</strong> Icon, Logo đơn giản, chữ viết.</li>' +
          '<li><strong class="text-slate-700">8 - 16 màu:</strong> Hình minh hoạ (Illustration), hoạt hình 2D.</li>' +
          '<li><strong class="text-red-500">32 - 64 màu (+):</strong> Ảnh chụp (Tốn thời gian xử lý, file nặng).</li>' +
          '</ul>' +
          '</div>' +

          '<button id="svg-tool-btn-convert" disabled class="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-black py-4 rounded-xl shadow-md transition active:scale-95 flex justify-center items-center gap-2 text-[13px] uppercase tracking-wide">' +
          '✨ Cắt & Tạo SVG' +
          '</button>' +
          '</div>' +
          ...
          '<div class="bg-white/90 backdrop-blur-md p-5 rounded-3xl shadow-sm border border-slate-100 space-y-4 relative svg-card">' +
          ...
          '<h3 class="font-bold text-blue-500 text-sm uppercase tracking-wide">Đang Vector Hóa...</h3>' +
          '<p class="text-[10px] text-slate-500 mt-1">Quá trình này có thể tốn vài giây</p>' +
          '</div>' +
          ...
          '<div class="w-full h-[320px] bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-2 flex items-center justify-center overflow-hidden svg-preview-box">' +
          ...
          '<textarea id="svg-tool-code" readonly class="svg-input w-full h-24 bg-blue-50/50 border border-blue-100 rounded-xl p-3 text-[11px] font-mono text-slate-700 outline-none custom-scrollbar" placeholder="<svg>...</svg>"></textarea>' +
          '</div>' +
    ...
    // ================= SCRIPT & LOGIC LOADER =================
    // Cần tải JS của thư viện bên ngoài (Cropper & ImageTracer) trước khi chạy logic
    ...
    // Sau khi tải xong thư viện, kích hoạt Logic hệ thống
    loadExternalScripts(function() {
        ...
        // Đổi màu Button Ratio khi Active
        ...
        // Xử lý Convert (Cắt + Vector Hóa)
        btnConvert.addEventListener('click', function() {
            if (!cropper) return;

            loadingLayer.style.display = 'flex'; // Hiện loading overlay
            ...
            setTimeout(() => {
                ...
                // Gọi ImageTracer (đã được tải qua script động)
                window.ImageTracer.imageToSVG(
                    ...
                    function(svgstr) {
                        ...
                        // Hiển thị Preview bằng Blob để an toàn cho DOM
