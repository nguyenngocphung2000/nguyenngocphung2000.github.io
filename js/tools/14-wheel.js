...
export function setupTool() {
    ...
    // ==========================================
    const safeSave = (key, val) => { try { window.localStorage.setItem(key, val); } catch(e) {} };
    const safeGet = (key) => { try { return window.localStorage.getItem(key); } catch(e) { return null; } };
    const safeRemove = (key) => { try { window.localStorage.removeItem(key); } catch(e) {} };
    ...
    const spinWheel = () => {
        ...
        const totalDegrees = 360 * (Math.floor(Math.random() * 6) + 7) + extraDegrees; 
    ...
    const calculateWinner = () => {
        ...
        let index = Math.floor(((winningDegree + sliceAngle / 2) % 360) / sliceAngle);
