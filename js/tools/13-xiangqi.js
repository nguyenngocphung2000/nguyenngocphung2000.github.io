// --- 13. Tool Cờ Tướng (Tuyệt Tác Neumorphism - Tỉ Số Độc Lập - Face-to-Face) ---
registerTool({
    id: 'tab-xiangqi',
    name: 'Cờ Tướng',
    icon: '♟️',
    html: '<style>' +
          ...
          /* Quân cờ Trắng Ngọc Trai nổi khối */
          ...
          /* Hiệu ứng Phát Sáng Neon khi bốc cờ */
    ...
    logic: function() {
        ...
        let board = [], turn = 'red', selected = null, isFlipped = false;
        let gameState = 'SETUP'; 
        let times = { red: 600, black: 600 }, timerId = null;
        
        // ĐIỂM SỐ GIỜ ĐƯỢC GẮN VÀO GHẾ NGỒI (TRÊN/DƯỚI) THAY VÌ MÀU CỜ
        ...
        // --- VẼ LƯỚI BÀN CỜ XÁM CHÌ ---
        const drawBoardSVG = () => {
            let svg = `<svg viewBox="0 0 900 1000" class="xq-svg-board">`;
            const strokeColor = "#94a3b8"; 
            for(let r=0; r<=9; r++) svg += `<line x1="50" y1="${50+r*100}" x2="850" y2="${50+r*100}" stroke="${strokeColor}" stroke-width="1.5"/>`;
            for(let c=0; c<=8; c++) {
                if(c===0 || c===8) svg += `<line x1="${50+c*100}" y1="50" x2="${50+c*100}" y2="950" stroke="${strokeColor}" stroke-width="1.5"/>`;
                else {
            ...
            const drawCross = (r, c) => {
                ...
                if(c > 0) res += `<path d="M${cx-d},${cy-d-l} L${cx-d},${cy-d} L${cx-d-l},${cy-d} M${cx-d},${cy+d+l} L${cx-d},${cy+d} L${cx-d-l},${cy+d}" fill="none" stroke="${strokeColor}" stroke-width="1.5"/>`;
                if(c < 8) res += `<path d="M${cx+d},${cy-d-l} L${cx+d},${cy-d} L${cx+d+l},${cy-d} M${cx+d},${cy+d+l} L${cx+d},${cy+d} L${cx+d+l},${cy+d}" fill="none" stroke="${strokeColor}" stroke-width="1.5"/>`;
                return res;
            };
            const pts = [[2,1], [2,7], [3,0], [3,2], [3,4], [3,6], [3,8], [7,1], [7,7], [6,0], [6,2], [6,4], [6,6], [6,8]];
            pts.forEach(p => { svg += drawCross(p[0], p[1]); });
            svg += `</svg>`;
        ...
        // HÀM BẬT/TẮT HỘP THOẠI ĐẦU HÀNG
        ...
        window.xqConfirmSurrender = (color) => {
            window.xqToggleSurrender(color, false); // Tắt hộp thoại
            const winnerName = color === 'red' ? 'Phe Đen' : 'Phe Đỏ';
            endGame(color === 'red' ? 'black' : 'red', `Phe ${color==='red'?'Đỏ':'Đen'} đã đầu hàng! ${winnerName} chiến thắng!`);
        };
        ...
        const updatePanelsUI = () => {
            ...
            const getHtml = (color, isTop, scoreVal) => {
                ...
                const bgCls = isRed ? 'bg-gradient-to-r from-red-50 to-rose-100 border-red-200' : 'bg-gradient-to-r from-slate-50 to-slate-200 border-slate-300';
                ...
                return `
                    <div class="relative overflow-hidden flex justify-between items-center ${bgCls} border p-4 rounded-2xl shadow-sm ${rotCls}">
                      ...
                      <div class="flex items-center gap-3">
                        ${isTop ? `<button onclick="window.xqToggleSurrender('${color}', true)" class="bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 text-[10px] font-bold px-3 py-2 rounded-xl shadow-sm transition active:scale-95">Đầu hàng</button>
            ...
            // Render điểm cho vị trí vật lý (Top/Bottom)
        ...
        const updateTimeHighlight = () => {
            ...
            if(tr) tr.className = `text-2xl font-mono font-bold px-3 py-1.5 rounded-xl w-24 text-center transition-all duration-300 bg-white/60 text-red-400 border border-red-200 shadow-inner`;
            if(tb) tb.className = `text-2xl font-mono font-bold px-3 py-1.5 rounded-xl w-24 text-center transition-all duration-300 bg-white/60 text-slate-400 border border-slate-300 shadow-inner`;
            ...
            for(let r = 0; r < 10; r++) {
                for(let c = 0; c < 9; c++) {
                    if (board[r][c]) {
                        ...
                        const isTopPlayer = (!isFlipped && piece.color === 'black') || (isFlipped && piece.color === 'red');
        ...
        const getPseudoMoves = (b, r1, c1) => {
            ...
            for(let r2 = 0; r2 < 10; r2++) {
                for(let c2 = 0; c2 < 9; c2++) {
                    ...
                    let v = false, dr = Math.abs(r2 - r1), dc = Math.abs(c2 - c1);
                    ...
                    switch(type) {
                        ...
                        case 'H':
                            if(dr===2 && dc===1) { if(!b[r1+Math.sign(r2-r1)][c1]) v = true; }
                            else if(dr===1 && dc===2) { if(!b[r1][c1+Math.sign(c2-c1)]) v = true; }
                            break;
                        case 'R':
                            if(dr===0 || dc===0) { if(countObs(b, r1, c1, r2, c2) === 0) v = true; }
                            break;
                        case 'C':
                            if(dr===0 || dc===0) {
        ...
        const countObs = (b, r1, c1, r2, c2) => {
            let c = 0;
            if(r1===r2) { let min=Math.min(c1,c2), max=Math.max(c1,c2); for(let i=min+1; i<max; i++) if(b[r1][i]) c++; } 
            else { let min=Math.min(r1,r2), max=Math.max(r1,r2); for(let i=min+1; i<max; i++) if(b[i][c1]) c++; }
            return c;
        };

        const isKingInCheck = (b, color) => {
            let kr = -1, kc = -1;
            const kingId = color === 'red' ? 'R_G' : 'B_G', enemyPfx = color === 'red' ? 'B' : 'R';
            
            for(let r=0; r<10; r++) {
                for(let c=0; c<9; c++) if(b[r][c] === kingId) { kr = r; kc = c; break; }
            }
            if(kr === -1) return false;

            const enemyKingId = color === 'red' ? 'B_G' : 'R_G';
            let ekr = -1;
            for(let r=0; r<10; r++) if(b[r][kc] === enemyKingId) { ekr = r; break; }
            if(ekr !== -1) {
                let obs = 0;
                for(let i=Math.min(kr, ekr)+1; i<Math.max(kr, ekr); i++) if(b[i][kc]) obs++;
        ...
        const getLegalMoves = (r, c) => {
            let pseudo = getPseudoMoves(board, r, c), piece = board[r][c], color = piece.startsWith('R') ? 'red' : 'black';
            return pseudo.filter(m => {
                let targetPiece = board[m.r][m.c];
                board[m.r][m.c] = piece; board[r][c] = null; 
                let inCheck = isKingInCheck(board, color);
                board[r][c] = piece; board[m.r][m.c] = targetPiece; 
                return !inCheck;
            });
        };
            ...
            for(let r=0; r<10; r++) {
                for(let c=0; c<9; c++) {
                    ...
                    if(p && p.startsWith(enemy === 'red' ? 'R' : 'B')) {
                        if(getLegalMoves(r, c).length > 0) { hasMoves = true; break; }
                    }
                }
                if(hasMoves) break;
            }
        ...
        const handleSquareClick = (r, c) => {
            ...
            if (selected) {
                if (getLegalMoves(selected.r, selected.c).some(m => m.r === r && m.c === c)) {
        ...
        const endGame = (winnerColor, msg) => {
            ...
            // CỘNG ĐIỂM DỰA VÀO VỊ TRÍ GHẾ NGỒI THAY VÌ MÀU CỜ
            if (winnerColor === 'red') {
                if (isFlipped) scores.top++; else scores.bottom++;
            } else {
                if (isFlipped) scores.bottom++; else scores.top++;
            }
        ...
        document.getElementById('xq-btn-flip').onclick = () => {
            // Chỉ đổi hiển thị bàn cờ, không ảnh hưởng điểm số của vị trí ghế ngồi
            isFlipped = !isFlipped;
            updatePanelsUI();
            renderPieces();
        };
